import { SkillRegistrySyncPreviewSchema } from "@sisyphus/ui/contracts";
...
export async function POST(request: NextRequest) {
  ...
  if (authentication.kind === "forbidden") {
    return apiFailure({ status: 403, error: "csrf_rejected", message: "The skill update review failed its origin or CSRF check." });
  }
  ...
  const result = await requestControlPlane({
    ...
    path: "/v1/skill-registry/sync/preview",
    method: "POST",
    parse: (payload) => SkillRegistrySyncPreviewSchema.parse(payload),
  });
