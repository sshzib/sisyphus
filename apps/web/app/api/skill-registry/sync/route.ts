import { SkillRegistrySyncResponseSchema } from "@sisyphus/ui/contracts";
...
export async function POST(request: NextRequest) {
  ...
  if (authentication.kind === "forbidden") {
    return apiFailure({ status: 403, error: "csrf_rejected", message: "The synchronization request failed its origin or CSRF check." });
  }
  ...
  const result = await requestControlPlane({
    ...
    path: "/v1/skill-registry/sync",
    method: "POST",
    parse: (payload) => SkillRegistrySyncResponseSchema.parse(payload),
  });
