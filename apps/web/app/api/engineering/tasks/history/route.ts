import { ClearEngineeringHistoryResponseSchema } from "@sisyphus/ui/contracts";
...
export async function DELETE(request: NextRequest) {
  ...
  if (authentication.kind === "forbidden") {
    return apiFailure({
      status: 403,
      error: "csrf_rejected",
      message: "The prompt-log cleanup failed its origin or CSRF check.",
    });
  }
  ...
  const result = await requestControlPlane({
    ...
    method: "DELETE",
    parse: (payload) => ClearEngineeringHistoryResponseSchema.parse(payload),
  });
